      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-white border-r border-gray-200 transition-transform duration-300 lg:static lg:translate-x-0 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-16 items-center justify-between border-b px-6">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-black tracking-tight text-gray-900">Sario<span className="text-primary">Vendor</span></span>
          </div>
        ...
        <div className="border-t border-gray-100 p-6">
          {vendor && (
            <div className="mb-4 px-1">
              <p className="text-xs font-bold text-gray-900 truncate">{vendor.businessName}</p>
              <span className={`mt-1 inline-block rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${STATUS_COLORS[vendor.status] ?? "bg-gray-100 text-gray-600"}`}>
                {vendor.status}
              </span>
            </div>
          )}
        ...
        <header className="h-16 shrink-0 border-b border-gray-200 bg-white/80 backdrop-blur-md px-6 flex items-center justify-between lg:justify-end">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-black text-gray-900">{vendor?.businessName ?? "Loading…"}</p>
              <p className="text-[10px] font-bold text-primary uppercase tracking-tighter">Vendor Portal</p>
            </div>
            <div className="h-10 w-10 rounded-xl bg-gray-100 border border-gray-200 overflow-hidden">
              <img src={`https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(vendor?.businessName ?? "V")}`} alt="avatar" />
            </div>
          </div>
        </header>
